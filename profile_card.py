...
def make_profile_card(
    ...
    if os.path.exists(bg_path):
        try:
            ...
            if os.path.basename(bg_path) == "archive_banner.png":
                ...
                bg = bg.crop((0, int(banner_h*0.05), W, banner_h + int(banner_h*0.05))).resize((W, banner_h), Image.Resampling.LANCZOS)
                banner = bg
            else:
    ...
    left_x = col_left + S(10)
    right_x = col_left + int((col_right - col_left) * 0.74)
    ...
    if badge:
        ...
        badge_x = min(name_bbox[2] + S(16), right_x - badge.size[0] - S(20))
    ...
    underline_y = rating_value_y + S(86)

    # Record aligned with name (same Y)
    record_label_y = name_y
    record_value_y = record_label_y + S(34)

    # Streak aligned with rating label (same Y)
    streak_label_y = rating_label_y
    streak_value_y = streak_label_y + S(32)

    rp_glow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    rgd = ImageDraw.Draw(rp_glow)
    rgd.rounded_rectangle((left_x - S(10), rating_value_y - S(8), left_x + S(320), rating_value_y + S(72)),
    ...
    draw_tracked(draw, (name_x, rating_label_y), "RATING", f_label, OFF_WHITE, S(3), stroke_width=S(1), stroke_fill=(0, 0, 0, 185))
    draw.text((name_x, rating_value_y), f"{pts} RP", font=f_big, fill=(rc[0], rc[1], rc[2], 255), stroke_width=S(2), stroke_fill=(0, 0, 0, 155))
    draw.line((name_x, underline_y, name_x + S(360), underline_y), fill=(rc[0], rc[1], rc[2], 48), width=S(2))
