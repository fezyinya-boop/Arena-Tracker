...
def make_profile_card(
    ...
    sig_w = (panel_pad_l - sig_x) - S(40)  # signature column width (kept out of stats panel)
    if sig_w > S(140):
        # Divider line
...
# HEADER = (185, 185, 200, 255)
    ...
    # WINRATE (right column, under record)
    wr_y = row1_y + S(112)
    draw_tracked(draw, (grid_right_x, wr_y), "WINRATE", font=f_lab, fill=HEADER, tracking=S(3), stroke_width=S(1), stroke_fill=(0, 0, 0, 180))
    draw.text(
        (grid_right_x, wr_y + S(40)),
        f"{wr}%",
        font=f_val,
        fill=WHITE,
        stroke_width=S(2),
    ...
    # STREAK (bottom-right)
    streak_y = row1_y + S(200)  # pushed down to avoid colliding with WINRATE + progress label
    draw_tracked(draw, (grid_right_x, streak_y), "STREAK", font=f_lab, fill=HEADER, tracking=S(3), stroke_width=S(1), stroke_fill=(0, 0, 0, 180))
    streak_col = (rc[0], rc[1], rc[2], 255) if streak >= 3 else WHITE
    draw.text(
        (grid_right_x, streak_y + S(44)),
    ...
    if next_rank_raw:
        # Use the progress label row for season branding (cleaner than "93% to X")
        season_txt = "Archive Arena • Season 1"
        draw.text((col_left, bar_y - S(24)), season_txt, font=f_small, fill=WHITE, stroke_width=S(2), stroke_fill=(0, 0, 0, 190))
    else:
