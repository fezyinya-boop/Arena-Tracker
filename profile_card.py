...
def draw_tracked_name(base_img, text_value, pos, font, tracking, fill=(236, 236, 240, 255),
    ...
    cx = x
    for i, ch in enumerate(chars):
        gd.text((cx, y + 1), ch, font=font, fill=glow_fill)
    ...
    for i, ch in enumerate(chars):
        ...
        # subtle metallic highlight near top edge
        d.text(
            (cx, y - 1),
            ch,
            font=font,
...
# Square off the top by extending full width - removes rounded top corners
    ...
    # Subtle scanline / holographic treatment to better match the digital arena aesthetic
    avatar_fx = Image.new("RGBA", (av_size, av_size), (0, 0, 0, 0))
    afd = ImageDraw.Draw(avatar_fx)
    for sy in range(0, av_size, S(4)):
        afd.line((0, sy, av_size, sy), fill=(255, 255, 255, 14), width=1)
    afd.ellipse((S(10), S(10), av_size - S(10), av_size - S(10)), outline=(255, 255, 255, 20), width=S(2))
    avatar_fx.putalpha(mask)
    card.paste(avatar_fx, (av_x, av_y), avatar_fx)
    ...
    if badge:
        ...
        badge_x = min(name_x + tracked_name_w + S(18), right_x - badge.size[0] - S(24))
