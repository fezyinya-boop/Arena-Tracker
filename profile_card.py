...
def make_profile_card(
    ...
    f_big   = load_font("Orbitron-VariableFont_wght.ttf", S(84))
    ...
    badge = get_rank_badge(current_rank_raw, size=S(64))
    badge_x = col_left
    if badge:
        card.paste(badge, (badge_x, header2_y - S(6)), badge)
        title_x = badge_x + S(76)
    else:
        title_x = badge_x
...
# HEADER = (185, 185, 200, 255)
    ...
    grid_right_x = col_mid + S(18)
    row1_y = content_top
    row2_y = content_top + S(110)
    ...
    draw.text((grid_right_x, row1_y + S(72)), f"{wr}% win rate", font=f_small, fill=SUB)

    # Progress
    bar_y = panel_pad_b - S(44)
