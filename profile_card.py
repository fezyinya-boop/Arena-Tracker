...
def make_profile_card(
    ...
    draw.line((0, banner_h - S(1), W, banner_h - S(1)), fill=(0, 0, 0, 90), width=S(2))
    ...
    # Main gold ring
    draw.ellipse(
        (av_x - S(3), av_y - S(3), av_x + av_size + S(3), av_y + av_size + S(3)),
        outline=(255, 200, 90, 255), width=S(5),
    )
    ...
    draw.ellipse(
        ...
        outline=(255, 245, 190, 60), width=S(1),
    )
    # Specular arc highlight on top of ring
    draw.arc(
        (av_x - S(3), av_y - S(3), av_x + av_size + S(3), av_y + av_size + S(3)),
        start=210, end=330, fill=(255, 245, 190, 115), width=S(2),
    )
    ...
    badge = get_rank_badge(current_rank_raw, size=S(54))
    if badge:
        # Vertically center badge on the actual rendered text
        text_center_y = name_draw_y + (name_bbox[3] - name_bbox[1]) // 2
        badge_y = text_center_y - badge.size[1] // 2 + S(13)
        badge_x = name_x + tracked_name_w + S(14)

        badge_shadow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
        bsd = ImageDraw.Draw(badge_shadow)
        bsd.ellipse(
            (badge_x + S(7), badge_y + S(10), badge_x + badge.size[0] - S(5), badge_y + badge.size[1] - S(2)),
            fill=(0, 0, 0, 70),
        )
        badge_shadow = badge_shadow.filter(ImageFilter.GaussianBlur(radius=S(3)))
        card = Image.alpha_composite(card, badge_shadow)
        draw = ImageDraw.Draw(card)
    ...
    draw.line((0, footer_glass_y, W, footer_glass_y), fill=(255, 255, 255, 26), width=S(1))
    draw.line((0, footer_glass_y + S(2), W, footer_glass_y + S(2)), fill=(0, 0, 0, 70), width=S(1))

    draw.text((col_left, label_y), footer_label, font=f_small, fill=(248, 248, 250, 235),
    ...
    draw.rounded_rectangle((col_left, bar_top, col_left + bar_w, bar_bottom),
                            radius=S(8), fill=(18, 20, 28, 220), outline=(255, 255, 255, 20), width=S(1))
    draw.line((col_left + S(6), bar_top + S(1), col_left + bar_w - S(6), bar_top + S(1)),
              fill=(255, 255, 255, 24), width=S(1))
    ...
    if next_rank_raw:
        ...
        if next_badge:
            ...
            draw.rounded_rectangle(
                (anchor_x - S(8), anchor_y - S(6), anchor_x + next_badge.size[0] + S(8), anchor_y + next_badge.size[1] + S(6)),
                radius=S(12), fill=(10, 12, 20, 150), outline=(255, 255, 255, 18), width=S(1),
            )
