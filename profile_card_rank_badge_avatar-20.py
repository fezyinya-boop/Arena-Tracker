...
def make_profile_card(
    ...
    name_y = top_header_y + S(4)  # high up, tagline fits comfortably below
    name_x = left_x + S(230)
    ...
    # Try italic variants in order until one loads
    _tagline_size = S(24)
    _tagline_candidates = [
    ...
    f_tagline = None
    for _tc in _tagline_candidates:
        import os as _os2
        _tp = _os2.path.join(FONTS_DIR, _tc)
        if _os2.path.exists(_tp):
            try:
                import PIL.ImageFont as _IF
                f_tagline = _IF.truetype(_tp, _tagline_size)
                break
            except Exception:
                pass
    if f_tagline is None:
        f_tagline = load_font("RussoOne-Regular.ttf", _tagline_size)
    ...
    LABEL = (255, 200, 90, 255) if _rank_clean in ("DIAMOND", "PLATINUM", "SILVER") else (242, 242, 245, 255)
    ...
    # Tagline (p_move) in italic Inter below the name
    if p_move:
        tagline_text = str(p_move)[:28]
        tagline_y = name_draw_y + f_name.size + S(6)
        tagline_col = (220, 220, 225, 180)
        draw.text((name_x, tagline_y), tagline_text, font=f_tagline, fill=tagline_col,
                  stroke_width=S(1), stroke_fill=(0, 0, 0, 100))
    ...
    rating_label_y = name_y + S(138)  # compensates for name moving up
    rating_value_y = rating_label_y + S(34)
    record_label_y = name_y + S(38)  # compensates for name moving up
