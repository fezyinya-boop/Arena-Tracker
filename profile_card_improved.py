...
def make_profile_card(
    ...
    # Accent strip
    draw.rounded_rectangle((S(22), S(20), W - S(22), S(26)), radius=S(6), fill=(rc[0], rc[1], rc[2], 185))
    ...
    f_small = load_font("Inter-VariableFont_opsz,wght.ttf", S(17))
    f_value = load_font("Orbitron-VariableFont_wght.ttf", S(30))
    f_big = fit_font(draw, f"{pts} RP", "Orbitron-VariableFont_wght.ttf", max_w=S(250), start_size=S(66), min_size=S(42))
    ...
    draw.text((title_x, header2_y + S(30)), "Archive Arena • Season 1", font=f_brand, fill=GOLD, stroke_width=S(2), stroke_fill=(0, 0, 0, 165))

    # Left bio block
    sig_x = av_x + S(2)
    sig_y = av_y + av_size + S(20)
    sig_w = (panel_x1 - S(42)) - sig_x
    draw.rounded_rectangle((sig_x, sig_y - S(12), sig_x + sig_w, sig_y + S(96)), radius=S(18), fill=(16, 16, 22, 210), outline=PANEL_LINE, width=S(2))
    ...
    move_text = clamp_text(draw, move_text, fit_font(draw, move_text, "Inter-VariableFont_opsz,wght.ttf", sig_w - S(32), S(22), S(14)), sig_w - S(32))
    f_move = fit_font(draw, move_text, "Inter-VariableFont_opsz,wght.ttf", sig_w - S(32), S(22), S(14))
    draw.text((sig_x + S(16), sig_y + S(38)), move_text, font=f_move, fill=WHITE if p_move else SUB, stroke_width=S(2), stroke_fill=(0, 0, 0, 165))

    # Stat grid
    total = wins + losses
    wr = round((wins / total) * 100) if total > 0 else 0
    content_top = panel_y + S(34)
    left_x = col_left
    right_x = col_left + int((col_right - col_left) * 0.62)
    row1_y = content_top + S(110)
    row2_y = row1_y + S(128)

    def stat_block(x: int, y: int, label: str, value: str, *, value_font: ImageFont.ImageFont, value_fill: RGBA = WHITE, subtext: Optional[str] = None) -> None:
        draw_tracked(draw, (x, y), label, f_label, OFF_WHITE, S(3), stroke_width=S(1), stroke_fill=(0, 0, 0, 185))
        draw.text((x, y + S(34)), value, font=value_font, fill=value_fill, stroke_width=S(2), stroke_fill=(0, 0, 0, 155))
        if subtext:
            draw.text((x, y + S(92)), subtext, font=f_small, fill=SUB)
    ...
    stat_block(left_x, row2_y, "WIN RATE", f"{wr}%", value_font=f_value, value_fill=WHITE)
    streak_fill = (rc[0], rc[1], rc[2], 255) if streak >= 3 else WHITE
    streak_text = f"+{streak}" if streak > 0 else str(streak)
    stat_block(right_x, row2_y, "STREAK", streak_text, value_font=f_value, value_fill=streak_fill)

    # Progress/footer strip
    bar_y = panel_y2 - S(56)
    pct_clamped = max(0.0, min(1.0, float(pct)))
    progress_label = "Archive Arena • Ranked Profile"
    if next_rank_raw:
        progress_label = f"{int(pct_clamped * 100)}% to {clean_rank_name(next_rank_raw)}"
    else:
        progress_label = "MAX RANK REACHED"

    draw.text((col_left, bar_y - S(28)), progress_label, font=f_small, fill=GOLD if next_rank_raw else (rc[0], rc[1], rc[2], 255))

    badge_slot = S(56)
    bar_w = (col_right - col_left) - badge_slot - S(12)
    bar_h = S(12)
    ...
    if next_rank_raw:
        ...
        if next_badge:
            card.paste(next_badge, (col_left + bar_w + S(16), bar_y + bar_h // 2 - (badge_slot - S(6)) // 2), next_badge)
