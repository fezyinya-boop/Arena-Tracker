...
def draw_tracked_name(base_img, text_value, pos, font, tracking,
    ...
    for i, ch in enumerate(chars):
        # Deep shadow - gives lift off the panel
        d.text((cx + 2, y + 3), ch, font=font, fill=(0, 0, 0, 160))
        # Mid shadow for soft depth
        d.text((cx + 1, y + 1), ch, font=font, fill=(0, 0, 0, 90))
        # Main silver body - bright, not dark
        d.text((cx, y), ch, font=font,
               fill=(210, 215, 225, 255),
               stroke_width=stroke_width,
               stroke_fill=(40, 44, 54, 180))
        # Specular highlight - top edge catches light (brushed metal effect)
        d.text((cx, y - 1), ch, font=font, fill=(255, 255, 255, 120))
        # Secondary shimmer - slightly offset for holographic depth
        d.text((cx - 1, y), ch, font=font, fill=(200, 210, 230, 40))
...
def make_profile_card(
    ...
    LABEL: RGBA = (180, 182, 190, 255)  # dimmed label color for visual hierarchy
    ...
    tracked_name_w = draw_tracked_name(
        card, name_text, (name_x, name_y - S(5)), f_name, S(16),
    ...
    rgd.text((name_x + S(3), rating_value_y + S(4)), num_text, font=f_big,
             fill=(rc[0], rc[1], rc[2], 120), stroke_width=S(2), stroke_fill=(0, 0, 0, 0))
    rgd.text((rp_x + S(3), rp_y + S(4)), "RP", font=f_rp,
             fill=(rc[0], rc[1], rc[2], 90), stroke_width=S(1), stroke_fill=(0, 0, 0, 0))
    ...
    # Bottom-right badge rank glow ring
    badge_icon = get_rank_badge(current_rank_raw, size=S(44))
    badge_icon_x = W - S(60)
    badge_icon_y = H - S(62)
    if badge_icon:
        # Rank-color glow behind badge icon
        bg_glow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
        bg = ImageDraw.Draw(bg_glow)
        bg.ellipse(
            (badge_icon_x - S(6), badge_icon_y - S(6),
             badge_icon_x + S(44) + S(6), badge_icon_y + S(44) + S(6)),
            fill=(rc[0], rc[1], rc[2], 55),
        )
        bg_glow = bg_glow.filter(ImageFilter.GaussianBlur(radius=S(6)))
        card = Image.alpha_composite(card, bg_glow)
        card.paste(badge_icon, (badge_icon_x, badge_icon_y), badge_icon)
