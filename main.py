...
def queue_list():
    ...
    rows = c.fetchall()
    conn.close()
    return [r[0] for r in rows]  # user_id stored as TEXT, keep as string
...
class MatchFoundView(discord.ui.View):
    ...
    def __init__(self, p1, p2):
        ...
        self.p2 = p2
        self.accepted = set()
        self.message = None  # stored after sending so timeout can edit it

    async def on_timeout(self):
        # Remove both players from queue so they don't stay stuck
        queue_remove(self.p1.id)
        queue_remove(self.p2.id)
        if self.message:
            try:
                await self.message.edit(
                    content=f"⌛ Match between {self.p1.mention} and {self.p2.mention} expired — neither player responded in time. Both removed from queue.",
                    view=None
                )
            except Exception:
                pass
    ...
    async def decline(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id not in [self.p1.id, self.p2.id]:
            return await interaction.response.send_message("This match isn't for you.", ephemeral=True)

        decliner = interaction.user
        other = self.p2 if interaction.user.id == self.p1.id else self.p1

        queue_remove(decliner.id)
        self.stop()

        await interaction.response.edit_message(
            content=f"❌ **{decliner.display_name}** declined the match. {other.mention} has been returned to the queue.",
            view=None
        )
...
async def try_queue_match(guild):
    ...
    p1 = guild.get_member(int(players[0]))
    p2 = guild.get_member(int(players[1]))
...
class MatchReportingView(discord.ui.View):
    ...
    async def finalize(self, interaction, winner_id, forfeit=False):
        ...
        conn = get_conn()
        c = conn.cursor()
        notes = "Auto-Forfeit: No response" if forfeit else ("Standard Match" if self.ranked else "Unranked Match")
        ...
        if self.ranked:
            ...
            result_value = f"📈 **{w_mem.display_name}**: `+{pts} RP`\n📉 **{l_mem.display_name}**: `-{pts} RP`"
            footer = "Arena Guide • Meta Data Recorded"
        else:
            # Unranked — no RP changes, no role updates
            rank_info = get_rank_info(1000)  # neutral color
            result_value = f"🏅 **{w_mem.display_name}** wins!\n*(Unranked — no RP awarded)*"
            footer = "Arena Guide • Unranked Match"
        ...
        embed.add_field(name="RESULTS", value=result_value, inline=False)
...
class MatchView(discord.ui.View):
    def __init__(self, match_id, p1, p2):
        super().__init__(timeout=7200.0)  # 2 hours, matches MatchReportingView
...
class OpenMatchView(discord.ui.View):
    ...
    async def join(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Reuse existing MatchReportingView and DeckSelect — unranked, no RP changes
        view = MatchReportingView(self.challenger, opponent, match_id, ranked=False)
...
class ChallengeView(discord.ui.View):
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.p1.id:
            return await interaction.response.send_message("Only the challenger can cancel.", ephemeral=True)
        self.stop()
        await interaction.response.edit_message(
            content=f"🚫 **{self.p1.display_name}** cancelled the challenge.",
            embed=None, view=None
        )
...
async def intro(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            "The definitive home for competitive online Grand Archive play.\n\n"
            "This server is powered by a custom **Arena Guide Bot** "
            "that monitors every match, calculates Elo (RP), and tracks the global meta. "
            "Automated Tournaments, Cash Prizes, Rankes Ladders, and more."
        ),
    ...
    matchmaking = (
        ...
        "• Use `/duel @user` to initiate a duel, /match to post a public lobby, and /queue for an instant ranked match.\n"
...
async def backup(ctx):
    """Sends a safe copy of the database file as a backup."""
    ...
    try:
        ...
        file = discord.File(backup_path, filename="arena_backup.db")
...
def home():
    ...
    return "Arena Guide API is Online"
...
# ===================== !commands — categorized help embed =====================
@bot.command(name="commands", aliases=["help", "cmds"])
async def commands_list(ctx):
    embed = discord.Embed(
        title="📖 ARCHIVE ARENA — COMMAND LIST",
        description="All commands use `/`. Admin-only commands are marked 🔒.",
        color=0x2b2d31
    )
    ...
    embed.add_field(
        name="⚔️ MATCHMAKING",
        value=(
            "`/match` — Post an open lobby (unranked)\n"
            "`/duel @user` — Challenge someone to a ranked duel\n"
            "`/queue` — Join matchmaking queue\n"
            "`/dequeue` — Leave matchmaking queue\n"
            "`/settle @winner @loser` — 🔒 Resolve a disputed match"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="📊 STATS & RANKS",
        value=(
            "`/rank [@user]` — View rank, RP, and record\n"
            "`/ranks` — Show all ranking tiers and RP thresholds\n"
            "`/profile [@user]` — View full profile card\n"
            "`/history` — View your last 10 matches\n"
            "`/leaderboard` — Refresh the top 10 leaderboard\n"
            "`/setprofile` — Set your title, signature move, or color"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="🃏 META & DECKS",
        value=(
            "`/meta` — View deck matchup meta snapshot\n"
            "`/decklist [query]` — Browse or search archetypes\n"
            "`/card <name>` — Look up a Grand Archive card"
        ),
        inline=False
    )
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`/tourney_start` — 🔒 Generate bracket and start\n"
            "`/tourney_reward @1st @2nd @3rd` — 🔒 Award RP to top 3\n"
            "`/tourney_add @user` — 🔒 Manually add a player\n"
            "`/tourney_kick @user` — 🔒 Remove a player\n"
            "`/tourney_list` — Show current roster\n"
            "`/tourney_end` — 🔒 End and wipe tournament"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="💸 PAYOUTS",
        value=(
            "`/register $tag` — Link your Cash App cashtag\n"
            "`/unregister` — Remove your cashtag\n"
            "`/payout_info` — How payouts work\n"
            "`/payout @user` — 🔒 Get a player's cashtag via DM"
        ),
        inline=False
    )
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`!intro` — 🔒 Post the arena intro embed"
        ),
        inline=False
    )

    embed.set_footer(text="Archive Arena • Use /commands anytime for this list")
...
# --- /ranks ---
@bot.tree.command(name="ranks", description="Show all ranking tiers and RP thresholds.")
...
# --- /setprofile ---
@bot.tree.command(name="setprofile", description="Set your profile title, signature move, or color.")
@app_commands.describe(
    field="Field to update: title, move, or color",
    value="The value to set (color must be hex, e.g. ff0000)"
)
@app_commands.choices(field=[
    app_commands.Choice(name="Title", value="title"),
    app_commands.Choice(name="Signature Move", value="move"),
    app_commands.Choice(name="Color (hex)", value="color"),
])
async def setprofile_slash(interaction: discord.Interaction, field: str, value: str):
    db_field = {"move": "signature_move", "title": "title", "color": "embed_color"}[field]

    if field == "color":
        value = value.lstrip("#").lstrip("0x")
        if len(value) != 6 or not all(c in "0123456789abcdefABCDEF" for c in value):
            return await interaction.response.send_message(
                "❌ Color must be a valid hex code, e.g. `ff0000` or `#ff0000`.", ephemeral=True
            )

    conn = get_conn()
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO profiles (user_id) VALUES (?)", (str(interaction.user.id),))
    c.execute(f"UPDATE profiles SET {db_field} = ? WHERE user_id = ?", (value, str(interaction.user.id)))
    conn.commit()
    conn.close()
    await interaction.response.send_message(f"✅ Your **{field}** has been updated to: `{value}`", ephemeral=True)


# --- Tournament slash commands ---
@bot.tree.command(name="tourney_open", description="(Mods) Open tournament registration.")
async def tourney_open_slash(interaction: discord.Interaction):
    if not _is_mod_or_admin(interaction.user):
        return await interaction.response.send_message("🚫 Mod role required.", ephemeral=True)
    ...
    async def join_callback(btn_interaction: discord.Interaction):
        if btn_interaction.user in tournament_players:
            return await btn_interaction.response.send_message("You're already in!", ephemeral=True)
        tournament_players.append(btn_interaction.user)
        embed.description = (
            f"Click the button below to enter the Archive Arena Tournament!\n\n"
...
@bot.tree.command(name="tourney_start", description="(Mods) Generate bracket and start the tournament.")
async def tourney_start_slash(interaction: discord.Interaction):
    if not _is_mod_or_admin(interaction.user):
        return await interaction.response.send_message("🚫 Mod role required.", ephemeral=True)

    global tournament_bracket, tournament_players
    if len(tournament_players) < 2:
        return await interaction.response.send_message("❌ Not enough players to start!", ephemeral=True)

    player_data = [(p, get_or_create_user(p.id, p.display_name)[2]) for p in tournament_players]
    player_data.sort(key=lambda x: x[1], reverse=True)
    sorted_players = [p[0] for p in player_data]

    bracket_size = 1 << (len(sorted_players) - 1).bit_length()
    tournament_bracket = []
    ...
    for i in range(bracket_size // 2):
        ...
        m = {"p1": p1, "p2": p2, "winner": None}
        if p2 is None: m["winner"] = p1
        if p1 is None: m["winner"] = p2
        tournament_bracket.append(m)
...
@bot.tree.command(name="tourney_reward", description="(Mods) Award RP to tournament top 3.")
@app_commands.describe(first="1st place", second="2nd place", third="3rd place")
async def tourney_reward_slash(interaction: discord.Interaction, first: discord.Member, second: discord.Member, third: discord.Member):
    if not _is_mod_or_admin(interaction.user):
        return await interaction.response.send_message("🚫 Mod role required.", ephemeral=True)
    ...
    for member, amt in rewards.items():
        ...
        hist.append(f"W:Tournament:{amt}")
        update_user_stats(member.id, new_pts, data[3] + 1, data[4], data[5] + 1, hist)
        summary += ("🥇" if amt == 150 else "🥈" if amt == 75 else "🥉")
    ...
    embed.set_footer(text="Archive Arena • Tournament Complete")
    await interaction.followup.send(embed=embed)
    await refresh_leaderboard(interaction.guild)


@bot.tree.command(name="tourney_add", description="(Mods) Manually add a player to the tournament.")
@app_commands.describe(member="Player to add")
async def tourney_add_slash(interaction: discord.Interaction, member: discord.Member):
    if not _is_mod_or_admin(interaction.user):
        return await interaction.response.send_message("🚫 Mod role required.", ephemeral=True)
    if not tournament_active:
        return await interaction.response.send_message("❌ No tournament is open. Use `/tourney_open` first.", ephemeral=True)
    if member in tournament_players:
        return await interaction.response.send_message(f"⚠️ {member.display_name} is already on the list.", ephemeral=True)
    tournament_players.append(member)
    await interaction.response.send_message(f"✅ **{member.display_name}** added to the roster.")


@bot.tree.command(name="tourney_kick", description="(Mods) Remove a player from the tournament.")
@app_commands.describe(member="Player to remove")
async def tourney_kick_slash(interaction: discord.Interaction, member: discord.Member):
    if not _is_mod_or_admin(interaction.user):
        return await interaction.response.send_message("🚫 Mod role required.", ephemeral=True)
    if member in tournament_players:
        tournament_players.remove(member)
        await interaction.response.send_message(f"✅ **{member.display_name}** removed from the tournament.")
    else:
        await interaction.response.send_message(f"❌ {member.display_name} isn't in the tournament list.", ephemeral=True)


@bot.tree.command(name="tourney_list", description="Show all players currently in the tournament.")
async def tourney_list_slash(interaction: discord.Interaction):
    if not tournament_active:
        return await interaction.response.send_message("No tournament is currently active.", ephemeral=True)
    if not tournament_players:
        return await interaction.response.send_message("The tournament is open but no one has joined yet.", ephemeral=True)
    player_list = "\n".join([f"• {p.display_name}" for p in tournament_players])
    embed = discord.Embed(title="📝 CURRENT ROSTER", description=player_list, color=0x3498db)
    embed.set_footer(text="Archive Arena")
    await interaction.response.send_message(embed=embed)


@bot.tree.command(name="tourney_end", description="(Mods) End the tournament and wipe all data.")
async def tourney_end_slash(interaction: discord.Interaction):
    if not _is_mod_or_admin(interaction.user):
        return await interaction.response.send_message("🚫 Mod role required.", ephemeral=True)
    if not tournament_active:
        return await interaction.response.send_message("There is no active tournament to end.", ephemeral=True)
    ...
    embed = discord.Embed(
        ...
        description="The tournament session has been ended. All data has been wiped.",
        color=0x95a5a6
    )
    await interaction.response.send_message(embed=embed)
...
async def settle_slash(interaction: discord.Interaction, winner: discord.Member, loser: discord.Member):
    # Check by Mod role ID rather than a specific Discord permission
    if not any(r.id == MOD_ROLE_ID for r in interaction.user.roles):
        return await interaction.response.send_message("🚫 You need the Mod role to use this.", ephemeral=True)
    ...
    try:
        ...
        await interaction.followup.send(
            f"❌ Something went wrong while settling: `{e}`",
            ephemeral=True
        )
        raise
